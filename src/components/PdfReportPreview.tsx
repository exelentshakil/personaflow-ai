...
export function PdfReportPreview({ report, onLogEvent }: PdfReportPreviewProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Top Action Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl bg-[var(--color-panel)] border border-[var(--color-border)] shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm sm:text-base font-bold text-[var(--color-text-primary)]">
              Personalized PDF Dossier &amp; Delivery Hub
            </h3>
            <p className="text-xs text-[var(--color-text-muted)]">
              Report Ref: <span className="font-mono font-semibold">{report.reportCode}</span> •{" "}
              Synthesized in {report.providerTelemetry.latencyMs}ms ({report.providerTelemetry.provider})
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap shrink-0">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 shadow-sm transition-all whitespace-nowrap"
          >
          ...
          <button
            ...
            disabled={emailSent}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-[var(--color-panel-subtle)] text-[var(--color-text-primary)] border border-[var(--color-border)] hover:bg-[var(--color-panel)] transition-all whitespace-nowrap"
          >
            {emailSent ? (
              <>
      ...
      <div
        id="printable-report"
        className="p-6 sm:p-10 rounded-2xl bg-white text-slate-900 border border-slate-200 shadow-lg space-y-8 font-sans max-w-4xl mx-auto"
      >
        {/* PDF Header Block */}
        <div className="border-b-2 border-slate-900 pb-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <div className="text-right sm:border-l sm:border-slate-200 sm:pl-4 shrink-0">
            ...
            <div className="inline-flex items-center gap-1 mt-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 border-b border-slate-200 pb-1.5">
            Calibrated Core Recommendations &amp; Action Items
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {report.coreInsights.map((insight, idx) => (
              ...
              >
                ...
                <div className="pt-2.5 border-t border-slate-100 text-xs text-blue-900 font-medium bg-blue-50/60 p-2 rounded-lg">
                  <strong className="text-blue-700 block text-[11px] uppercase">Protocol Mandate:</strong>
                  {insight.actionItem}
                </div>
              </div>
            ))}
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 border-b border-slate-200 pb-1.5">
            90-Day Phased Execution Trajectory
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {report.personalizedRoadmap.map((phase, idx) => (
              ...
              >
                ...
                <div className="text-xs font-semibold text-slate-700 bg-white p-2 rounded border border-slate-200">
        ...
        {/* Footer & Telemetry */}
        <div className="pt-6 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-500 font-mono">
          <div>
